...
export default function SeleccionPoblacion() {
    ...
    return (
        <div className="relative min-h-screen flex flex-col font-['Inter',sans-serif] select-none overflow-hidden bg-[#1B4332] selection:bg-[#39A900] selection:text-white">
            ...
            <header className="relative w-full flex justify-between items-center px-6 md:px-10 py-4 md:py-6 z-20">
                <motion.div 
                    initial={{ x: -30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}
                    className="flex items-center bg-white/10 backdrop-blur-md px-4 md:px-6 py-2 md:py-3 rounded-2xl border border-white/20 shadow-lg">
                    <img src="/logo-ape.png" alt="SENA Agencia Pública de Empleo" className="h-8 md:h-[40px] lg:h-[46px] object-contain drop-shadow-md brightness-0 invert" />
                </motion.div>
            ...
            <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 relative z-10 overflow-hidden">
                <motion.div 
                    ...
                    className="text-center mb-6 md:mb-10 lg:mb-16 mt-[-20px] md:mt-[-40px] max-w-4xl">
                    <h1 className="text-2xl md:text-4xl lg:text-[clamp(2rem,4vw,3.5rem)] font-black text-white tracking-tight leading-tight drop-shadow-[0_5px_15px_rgba(0,0,0,0.5)]">
                ...
                <motion.div 
                    ...
                    initial="hidden"
                    animate="show"
                    className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8 max-w-[1600px] w-full justify-center px-4">
                    ...
                    {cards.map((card, index) => {
                        ...
                        <motion.div key={index} variants={itemVariants} className="w-full flex">
                            <Link 
                                ...
                                <div className="flex-1 p-4 md:p-6 lg:p-10 flex flex-col items-center text-center pb-8 lg:pb-12 justify-center">
                                    <div className="w-16 h-16 md:w-[90px] md:h-[90px] lg:w-[110px] lg:h-[110px] rounded-[20px] md:rounded-[30px] rotate-3 bg-gradient-to-br from-white/20 to-white/5 flex items-center justify-center mb-4 md:mb-8 backdrop-blur-md group-hover:rotate-0 group-hover:scale-110 transition-all duration-300 shadow-lg border border-white/30">
                                        <Icon className="text-[#5ceb00] group-hover:text-white transition-colors w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14" strokeWidth={1.5} />
                                    </div>
                                    <h3 className="text-xl md:text-[28px] lg:text-[34px] font-black text-white mb-2 md:mb-4 leading-[1.1] tracking-tight">{card.title}</h3>
                                    <p className="text-xs md:text-[15px] lg:text-[16px] text-gray-300 font-medium leading-relaxed drop-shadow-md">{card.desc}</p>
                                </div>
