...
// --- Datos mock ---
const initialQueue = [
    { id: 't-042', turn: 'N-042', doc: '1.023.456.678', timeWait: '0 minutos', type: 'AtenciÃ³n Prioritaria', typeColor: 'bg-red-700', service: 'Tipo de atenciÃ³n N-042' },
    { id: 't-043', turn: 'N-043', doc: '1.023.456.678', timeWait: '20 minutos', type: 'AtenciÃ³n Prioritaria', typeColor: 'bg-red-700', service: 'Tipo de atenciÃ³n N-043' },
    { id: 't-044', turn: 'N-044', doc: '1.085.987.123', timeWait: '20 minutos', type: 'AtenciÃ³n Empresa', typeColor: 'bg-blue-700', service: 'Empresa: "Inversiones del Caribe"' },
    { id: 't-045', turn: 'N-045', doc: '1.144.331.890', timeWait: '45 minutos', type: 'AtenciÃ³n Interna SENA', typeColor: 'bg-green-700', service: 'Tipo de atenciÃ³n N-045' },
    { id: 't-046', turn: 'N-046', doc: '1.020.884.111', timeWait: '60 minutos', type: 'AtenciÃ³n Normal', typeColor: 'bg-gray-700', service: 'Tipo de atenciÃ³n N-046' }
];
...
const HISTORY = [
    ...
    { turn: 'N-040', doc: '1.085.118.112', type: 'Empresa', typeDetails: 'Inversiones del Caribe', duration: '--', obs: '--', status: 'NO ASISTIÃ“', statusColor: 'bg-red-100 text-red-700' },
    { turn: 'N-039', doc: '1.085.457.999', type: 'Prioritaria', typeDetails: '--', duration: '28:10', obs: 'OrientaciÃ³n de carrera', status: 'COMPLETADO', statusColor: 'bg-green-100 text-green-700' },
];

const STEP_DEFS = [
    { key: 'llamado',     label: 'Llamado',    Icon: PhoneCall },
    { key: 'checkin',     label: 'Check-in',   Icon: UserCheck },
    { key: 'consultoria', label: 'ConsultorÃ­a', Icon: MessageSquare },
...
export default function Dashboard({ auth }) {
    ...
    const avanzaTurnoAnimate = (estado) => {
        ...
        if (activeTurn) {
            const newHistory = {
                ...
                doc: activeTurn.doc,
                type: activeTurn.type.replace('AtenciÃ³n ', ''),
                typeDetails: activeTurn.service,
                duration: elapsed,
                obs: observaciones || '--',
                status: estado,
                statusColor: estado === 'COMPLETADO' ? 'bg-green-100 text-green-700' : (estado === 'NO ASISTIÃ“' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700')
            };
    ...
    // 1. Llamado automÃ¡tico al cargar el turno
    useEffect(() => {
    ...
    // 2. ConsultorÃ­a y check-in automÃ¡ticos al escribir observaciones
    useEffect(() => {
    ...
    const disableFinalizar = !activeTurn || showSuccessAnim; // Habilitado siempre que haya turno activo, y el boton de finalizar marcarÃ¡ el cierre automÃ¡ticamente
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F4F6F9] font-['Inter',sans-serif] text-gray-800 overflow-hidden">
            ...
            <header className="h-[5.5rem] bg-white shadow-sm flex items-center justify-between px-8 shrink-0 relative z-10 w-full">
                ...
                <div className="flex items-center gap-6">
                    <Link href={route('logout')} method="post" as="button" className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Cerrar SesiÃ³n">
                        <LogOut size={24} />
                    </Link>
            ...
            <main className="flex-1 overflow-x-hidden overflow-y-auto w-full p-6 lg:p-8 2xl:p-12 flex flex-col gap-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    {/* COLUMNA 2: AtenciÃ³n Actual */}
                    <div className="lg:col-span-6 flex flex-col gap-5 relative">
                        <h2 className="text-2xl lg:text-3xl font-bold text-gray-800 tracking-tight">AtenciÃ³n Actual</h2>
                        ...
                        <AnimatePresence mode="wait">
                            {activeTurn ? (
                                <motion.div 
                                        ...
                                        {showSuccessAnim && (
                                            <motion.div 
                                                ...
                                                <motion.div 
                                                    ...
                                                    <h2 className="text-4xl font-black text-[#1B4332] mt-4 tracking-tight">AtenciÃ³n Exitosa</h2>
                                    ...
                                    {/* Progreso (Steps) */}
                                    <div className="my-8">
                                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-5">PROGRESO DE LA ATENCIÃ“N</p>
                                        <div className="relative flex justify-between items-center w-full px-2">
                                            ...
                                            {STEP_DEFS.map((step, i) => {
                                                ...
                                                // AnimaciÃ³n de timbrado/llamada: max 10 repeticiones
                                    ...
                                    <div className="mt-auto flex flex-col pt-6">
                                        <label className="text-sm font-bold text-gray-600 mb-3 flex items-center gap-2">
                                            <Edit3 size={18} /> Observaciones de la AtenciÃ³n
                                        </label>
                                        <textarea 
                                            ...
                                            placeholder="Ingrese aquÃ­ los detalles y conclusiones de la atenciÃ³n brindada..."
                                            value={observaciones}
                                        ...
                                        <button 
                                            ...
                                            <ClipboardList size={24} /> FINALIZAR Y ARCHIVAR ATENCIÃ“N <ArrowRight size={24} className="group-hover:translate-x-2 transition-transform" />
                                        </button>
                                    </div>
                                </motion.div>
                            ) : (
                    ...
                    <div className="lg:col-span-3 flex flex-col gap-5">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <button
                                ...
                                <Ban size={26} className={disableNoAsistio ? "text-gray-400" : "text-red-500"} /> NO ASISTIÃ“
                            </button>
                            <button
                        ...
                        <div className="mt-8">
                            <div className="flex items-center justify-between mb-5">
                                <h3 className="font-black text-2xl text-gray-800 tracking-tight">EstadÃ­sticas</h3>
                        ...
                        <table className="w-full text-left font-['Inter'] whitespace-nowrap">
                            <thead>
                                <tr className="border-b-2 border-gray-100 bg-gray-50/50">
                                    {['TURNO', 'IDENTIFICACIÃ“N', 'TIPO', 'DURACIÃ“N', 'OBSERVACIONES', 'ESTADO', ''].map((h, i) => (
                                        ...
                                        <motion.tr 
                                            ...
                                            <td className="py-5 px-8 text-right">
                                                <button className="text-sm text-[#39A900] hover:text-[#2a7a00] font-black flex items-center gap-1 justify-end ml-auto group">
                                                    <span className="group-hover:translate-x-[-2px] transition-transform">Ver mÃ¡s</span> <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
                                                </button>
                                            </td>
                                        </motion.tr>
                                    ))}
